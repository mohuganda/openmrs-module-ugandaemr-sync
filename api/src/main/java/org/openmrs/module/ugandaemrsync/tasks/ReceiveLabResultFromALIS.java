...
public class ReceiveLabResultFromALIS extends AbstractTask {
    protected final Log log = LogFactory.getLog(ReceiveLabResultFromALIS.class);
    ...
    public void execute() {
        ...
        for (SyncTask syncTask : ugandaEMRSyncService.getIncompleteActionSyncTask(ALIS_TESTS_SYNC_TYPE_UUID)) {
    ...
    public Order getOrder(String assessionNumber) {
        ...
        List list = Context.getAdministrationService().executeSQL(String.format(ALIS_LAB_TEST_ORDER_QUERY, assessionNumber), true);
        if (list.size() > 0) {
            for (Object o : list) {
